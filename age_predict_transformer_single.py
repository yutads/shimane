...
from my_def import compute_loss_age
from my_def import compute_loss_sex
from my_def import train_step_single
from my_def import val_step_single
from my_def import test_step_single
from my_def import seed_worker
from my_def import train_model_multi

train_model = train_model_multi
...
dt_now = datetime.datetime.now()
with open("output/age_predict_transformer_single.txt", 'a') as f:
...
np.savez(f'../02_data_analysis/temp/single_smooth_{s}_1',
         test_pred_1,
         test_pred_corrected_1,
         test_label_1,
         test_idx_1)

np.savez(f'../02_data_analysis/temp/single_smooth_{s}_2',
         test_pred_2,
         test_pred_corrected_2,
         test_label_2,
         test_idx_2)
...
test_idx_dynamic_1 = nested_predict(data_dynamic,data_label,
                                    ...
                                    title=f'DynamicFC {s} single model1',
                                    batch_size = 8
                                   )
...
test_idx_dynamic_2 = nested_predict(data_dynamic,data_label,
                                    ...
                                    title=f'DynamicFC {s} single model2',
                                    batch_size = 8
                                   )
...
np.savez(f'../02_data_analysis/temp/single_smooth_dynamic_{s}_1',
         test_pred_dynamic_1,
         test_pred_dynamic_corrected_1,
         test_label_dynamic_1,
         test_idx_dynamic_1)
np.savez(f'../02_data_analysis/temp/single_smooth_dynamic_{s}_2',
         test_pred_dynamic_2,
         test_pred_dynamic_corrected_2,
         test_label_dynamic_2,
         test_idx_dynamic_2)

np.savez(f'../02_data_analysis/temp/single_smooth_{s}_dynamic_1',
         test_pred_dynamic_1,
         test_pred_dynamic_corrected_1,
         test_label_dynamic_1,
         test_idx_dynamic_1)
np.savez(f'../02_data_analysis/temp/single_smooth_{s}_dynamic_2',
         test_pred_dynamic_2,
         test_pred_dynamic_corrected_2,
         test_label_dynamic_2,
         test_idx_dynamic_2)